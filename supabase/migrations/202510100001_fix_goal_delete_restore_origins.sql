-- Migração: Corrigir fn_goal_delete_with_correct_logic para restaurar fundos às contas de origem quando progresso < 100% e evitar transações com valor < 0.01
-- Data: 2025-10-10
...
DECLARE
  ...
  v_objetivos_owner_id uuid;
  v_categoria_id uuid;
  v_final_result json;
  -- Novos: variáveis para restauro por conta de origem
  v_allocation record;
  v_origin_owner_id uuid;
  v_origin_family_id uuid;
  v_origin_categoria_id uuid;
BEGIN
  -- 1. Idempotência (se existir registo anterior para a mesma key)
  IF idempotency_key IS NOT NULL THEN
    SELECT result INTO v_final_result
    FROM idempotent_ops 
    WHERE operation_key = idempotency_key 
  ...
  -- Obter proprietário real da conta Objetivos (evita problemas de RLS)
  SELECT user_id INTO v_objetivos_owner_id
  FROM accounts
  WHERE id = v_objetivos_account_id;

  -- 5. Categoria "Objetivos" (para o proprietário da conta Objetivos)
  SELECT id INTO v_categoria_id
  FROM categories 
  WHERE nome = 'Objetivos' AND user_id = v_objetivos_owner_id
  LIMIT 1;
  IF v_categoria_id IS NULL THEN
    INSERT INTO categories (nome, user_id, family_id, cor)
    VALUES ('Objetivos', v_objetivos_owner_id, goal_record.family_id, '#3B82F6')
  ...
  IF v_goal_progress < 100 THEN
    -- Caso <100%: restaurar fundos às contas de origem e libertar reservas da conta Objetivos
    -- 7.1 Agregar alocações por conta de origem e criar receitas de restauro
    FOR v_allocation IN
      SELECT account_id, SUM(valor) AS amount
      FROM goal_allocations
      WHERE goal_id = goal_id_param
      GROUP BY account_id
    LOOP
      IF COALESCE(v_allocation.amount, 0) >= 0.01 THEN
        -- Obter proprietário e família da conta de origem
        SELECT user_id, family_id INTO v_origin_owner_id, v_origin_family_id
        FROM accounts
        WHERE id = v_allocation.account_id;

        -- Garantir categoria "Objetivos" para o proprietário da conta origem
        SELECT id INTO v_origin_categoria_id
        FROM categories
        WHERE user_id = v_origin_owner_id AND nome = 'Objetivos'
        LIMIT 1;
        IF v_origin_categoria_id IS NULL THEN
          INSERT INTO categories (nome, user_id, family_id, cor)
          VALUES ('Objetivos', v_origin_owner_id, v_origin_family_id, '#3B82F6')
          RETURNING id INTO v_origin_categoria_id;
        END IF;

        -- Criar receita na conta de origem para restaurar o valor alocado
        INSERT INTO transactions (
          account_id, categoria_id, valor, tipo, data, descricao, goal_id, user_id, family_id
        ) VALUES (
          v_allocation.account_id, v_origin_categoria_id, v_allocation.amount, 'receita', NOW()::date,
          'Restauro de objetivo (<100%)', goal_id_param, v_origin_owner_id, v_origin_family_id
        );
      END IF;
    END LOOP;

    -- 7.2 Libertar reserva na conta Objetivos (despesa) apenas se houver valor suficiente
    IF v_total_allocated >= 0.01 THEN
      INSERT INTO transactions (
        account_id, categoria_id, valor, tipo, data, descricao, goal_id, user_id, family_id
      ) VALUES (
        v_objetivos_account_id, v_categoria_id, v_total_allocated, 'despesa', NOW()::date,
        'Remoção de objetivo (<100%)', goal_id_param, v_objetivos_owner_id, goal_record.family_id
      );
    END IF;

    -- 7.3 Remover alocações (após registar restauro)
  ...
  v_final_result := json_build_object(
    ...
    'message', CASE 
      ...
      WHEN v_goal_progress < 100 THEN 'Objetivo eliminado: reservas libertadas e fundos restaurados às contas de origem'
  ...
  -- 10. Idempotência
  IF idempotency_key IS NOT NULL THEN
    INSERT INTO idempotent_ops (operation_key, user_id, operation_type, operation_data, result)
    VALUES (
...
COMMENT ON FUNCTION public.fn_goal_delete_with_correct_logic IS 'Elimina objetivo: <100% restaura fundos às contas de origem e liberta reservas; =100% corrige duplicação (não debita contas de origem; só liberta reservas).';
