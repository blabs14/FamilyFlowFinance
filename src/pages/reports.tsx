...
const ReportsPage = () => {
  ...
  const { prevIncome, prevExpenses, prevNet, incomeDelta, expensesDelta, netDelta } = useMemo(() => {
    const income = prevFilteredTransactions.filter(t => t.tipo === 'receita').reduce((s, t) => s + ((t.amount_cents || 0) / 100), 0);
    const expenses = prevFilteredTransactions.filter(t => t.tipo === 'despesa').reduce((s, t) => s + ((t.amount_cents || 0) / 100), 0);
