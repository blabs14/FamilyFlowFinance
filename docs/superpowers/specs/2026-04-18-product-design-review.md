...
- **Implicações — Edge Functions / Serviços / RPCs:**
  ...
  - RPC `split_transaction(transaction_id, splits jsonb)` — valida soma = amount; insert em `expense_splits`.
...
- **Parked-aceite:** (a) Lighthouse CI como PR gate (MVP: warning-only via Sentry metrics); (b) Grafana/Prometheus (Supabase Studio + Sentry suficientes no MVP); (c) MFA/2FA para admins (mirror de Unit 15); (d) OpenTelemetry tracing distribuído (pós-SaaS multi-tenant); (e) Chaos/load testing (pós-SaaS).
